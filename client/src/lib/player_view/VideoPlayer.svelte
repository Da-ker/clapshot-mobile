...
function debugReviewTap(_message: string) {}
...
function onCommentReviewHiddenTap(event: Event) {
    ...
    if (isSecondTap) {
        ...
        reviewPlayLockUntil = now + 560;
        exitCommentReviewTapMode();
        suppressOverlayRevealUntil = now + 560;
...
const WINDOW_KEY_ACTIONS: {[key: string]: (e: KeyboardEvent)=>any} = {
        ...
        'ArrowUp': () => setVolume01(getVolume01() + 0.1),
        'ArrowDown': () => setVolume01(getVolume01() - 0.1),
