...
<div class="relative w-full min-w-0 rounded-xl border shadow-[0_2px_10px_rgba(0,0,0,0.18)] overflow-hidden {isCompleted ? 'border-sky-950/70 bg-slate-950/85' : (indent > 0 ? 'border-slate-700/80 bg-slate-900/35 border-l-[3px] border-l-slate-500/80' : 'border-slate-700/60')}">
    {#if canComplete}
    ...
    <div
        ...
        class="relative z-10 block box-border w-full min-w-0 max-w-full overflow-hidden text-ellipsis bg-gradient-to-b {isCompleted ? 'from-[#060b14] to-[#0b1220] hover:from-[#0a1220] hover:to-[#111b2e]' : 'from-slate-800 to-slate-900'} {!!comment.timecode && !isCompleted ? 'hover:from-slate-700 hover:to-slate-800' : ''}"
        tabindex="0"
        role="link"
