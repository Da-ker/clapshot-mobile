...
>
    ...
    <div class="p-2 flex items-center justify-between" transition:slide="{{ duration: 200 }}">
        ...
        <div class="flex-none">
            <button
                class="fa fa-link border rounded-lg px-2 py-1 text-sm h-11 border-gray-500 text-gray-300 hover:bg-gray-700"
        ...
        <div class="flex-1 flex justify-end">
            <button class="border rounded-lg px-1 placeholder: ml-2 text-sm h-11 border-cyan-500 text-cyan-500" onclick={()=>showReply=true}>{$t('comments.reply')}</button>
            {#if comment.userId == $curUserId || $curUserIsAdmin}
                <button class="border rounded-lg px-1 ml-2 text-sm h-11 border-cyan-600 text-cyan-600" onclick={()=>{editing=true;}}>{$t('comments.edit')}</button>
                {#if !hasChildren()}
                <button class="border rounded-lg px-1 ml-2 text-sm h-11 border-red-300 text-red-300" onclick={onClickDeleteComment}>{$t('comments.deleteShort')}</button>
                {/if}
            {/if}
        </div>
    </div>
