...
export class HybridVideoDecoder implements IVideoDecoder {
  ...
  private forceHtml5Only: boolean;
  ...
  constructor(config: HybridVideoDecoderConfig) {
    ...
    // Check config setting (defaults to true if not specified)
    const enableMediabunny = config.enableMediabunny !== false;
    const isSafari = HybridVideoDecoder.isSafariBrowser();

    // Safari-specific guard: keep using HTML5 path to avoid audio-only canvas failures.
    this.forceHtml5Only = isSafari;
    this.mediabunnyAvailable = typeof VideoDecoder !== 'undefined' && enableMediabunny && !this.forceHtml5Only;
  }

  private static isSafariBrowser(): boolean {
    if (typeof navigator === 'undefined') return false;

    const ua = navigator.userAgent || '';
    const hasSafari = /Safari/.test(ua);
    const isWebkitChrome = /Chrome|Chromium|CriOS|EdgiOS|OPiOS|FxiOS/.test(ua);
    const isWeChat = /MicroMessenger/i.test(ua);

    // Target real Safari only; exclude WeChat WebView and other browsers.
    return hasSafari && !isWebkitChrome && !isWeChat;
  }
  ...
  private async ensureMediabunny(): Promise<boolean> {
    if (this.forceHtml5Only) return false;
  ...
  async stepFrame(direction: 1 | -1, count = 1): Promise<FramePosition> {
    // Safari hard lock: always use HTML5 path
    if (this.forceHtml5Only) {
      this.switchToHtml5();
