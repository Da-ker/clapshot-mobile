...
function installThumbScrubber(el: HTMLElement)
{
...
function removeThumbScrubber(el: HTMLElement)
{
...
function onPointerEnter(e: PointerEvent) {
    if (e.pointerType === 'mouse' || e.pointerType === 'pen') {
        installThumbScrubber(e.currentTarget as HTMLElement);
    }
}

function onPointerLeave(e: PointerEvent) {
    removeThumbScrubber(e.currentTarget as HTMLElement);
}
</script>

<div class="w-full aspect-video mx-auto rounded-md overflow-hidden"
  style="background-image: url('{thumbPosterUrl}'); background-size: cover; background-position: 0 0; touch-action: manipulation; {extra_styles}"
  role="img"
  onblur={()=>{}}
  onfocus={()=>{}}
