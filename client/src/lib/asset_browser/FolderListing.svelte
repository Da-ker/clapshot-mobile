...
import { createEventDispatcher, tick, mount, unmount, onMount } from "svelte";
...
// Touch / mobile behavior: prefer single-tap open, avoid drag/select conflicts.
let prefersTouchOpen = $state(false);
let touchOpenGuard = $state<{ id: string; at: number } | null>(null);

function updateTouchPreference() {
    if (typeof window === 'undefined' || typeof navigator === 'undefined') {
        prefersTouchOpen = false;
        return;
    }
    const coarsePointer = window.matchMedia('(hover: none), (pointer: coarse)').matches;
    const hasTouch = (navigator.maxTouchPoints ?? 0) > 0;
    prefersTouchOpen = coarsePointer || hasTouch;
}

onMount(() => {
    updateTouchPreference();
    const mq = window.matchMedia('(hover: none), (pointer: coarse)');
    const onChange = () => updateTouchPreference();
...
function handleMouseUp(e: MouseEvent, item: VideoListDefItem) {
    ...
    if (prefersTouchOpen) return; // mobile: prioritize tap-to-open
...
function handleTileClick(item: VideoListDefItem, e: Event) {
    if (isDragging || e.defaultPrevented) {
        return;
    }

    // De-duplicate: on iOS you may get pointerup + click
    if (touchOpenGuard && touchOpenGuard.id === item.id && Date.now() - touchOpenGuard.at < 700) {
        touchOpenGuard = null;
        return;
    }

    // Preserve desktop multi-select semantics (modifier keys)
    if (!prefersTouchOpen && e instanceof MouseEvent) {
        const isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;
        const hasModifier = e.shiftKey || (isMac ? e.metaKey : e.ctrlKey) || e.altKey;
        if (hasModifier) {
            return;
        }
    }
    ...
    $selectedTiles = {};
    dispatchOpenItem(item.id);
}

function handlePointerUp(item: VideoListDefItem, e: PointerEvent) {
    if (!prefersTouchOpen || isDragging || e.defaultPrevented) {
        return;
    }
    ...
    touchOpenGuard = { id: item.id, at: Date.now() };
    $selectedTiles = {};
    dispatchOpenItem(item.id);
}
...
<div use:enterKeyInterceptor>
    <section
        use:dndzone="{{
            items, dragDisabled: dragDisabled || prefersTouchOpen,
        ...
        class="flex flex-wrap gap-3 md:gap-4 p-3 md:p-4 bg-slate-900 rounded-md"
        role="list"
    >
        {#each items as item(item.id)}
            <div
                ...
                onclick={stopPropagation((e) => { bubble('click')(e); handleTileClick(item, e); })}
                onpointerup={(e: PointerEvent) => handlePointerUp(item, e)}
...
:global(.video-list-tile-sqr) {
    width: clamp(8.5rem, 28vw, 10rem);
    height: clamp(8.5rem, 28vw, 10rem);
