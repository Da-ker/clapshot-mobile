...
<main>
    ...
    <div class="safe-area-pad box-border flex flex-col bg-[#101016] w-full h-[100dvh] overflow-hidden {debugLayout?'border-2 border-yellow-300':''}">
        ...
        <div class="relative h-full min-h-0 overflow-hidden">
            ...
            <div
                class="fixed md:absolute z-20 left-1/2 -translate-x-1/2 md:translate-x-0 w-[calc(100%-1rem)] max-w-[1400px] md:w-[26rem] md:max-w-none md:left-auto md:right-4 mt-0 bottom-[calc(var(--mobile-comment-input-h,88px)+env(safe-area-inset-bottom))] md:mt-0 md:bottom-[max(0.5rem,env(safe-area-inset-bottom))] h-[min(44dvh,var(--mobile-comment-max-h))] max-h-[min(44dvh,var(--mobile-comment-max-h))] md:h-auto md:max-h-none flex flex-col overflow-hidden rounded-t-2xl md:rounded-xl bg-[#0f1728]/88 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.38)] transition-all duration-200 mb-0 {commentsPanelOpen ? 'md:translate-y-0 md:opacity-100' : 'md:translate-y-6 md:opacity-0 md:pointer-events-none'} {commentsPanelMode === 'full' ? 'md:max-h-[85vh]' : ''}"
            ...
            <div bind:this={commentInputDockEl} class="fixed md:static left-1/2 -translate-x-1/2 md:translate-x-0 w-[calc(100%-1rem)] max-w-[1400px] md:w-auto md:max-w-none md:left-auto md:right-auto bottom-[max(0px,env(safe-area-inset-bottom))] z-40 p-2 rounded-t-xl md:rounded-none bg-[#0f1728]/92 backdrop-blur-md shadow-[0_-10px_24px_rgba(0,0,0,0.28)] md:shadow-none">
