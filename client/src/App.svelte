...
<main>
    ...
    <div class="safe-area-pad box-border flex flex-col bg-[#101016] w-full h-[100dvh] overflow-hidden {debugLayout?'border-2 border-yellow-300':''}">
        ...
        <div class="relative h-full min-h-0 overflow-hidden">
            ...
            <div class="hidden relative md:absolute z-20 left-2 right-2 md:left-auto md:right-4 mt-0 md:mt-0 bottom-0 md:bottom-[max(0.5rem,env(safe-area-inset-bottom))] md:w-[26rem] h-[clamp(13rem,34dvh,20rem)] md:h-auto flex flex-col overflow-hidden rounded-t-2xl md:rounded-xl bg-[#0f1728]/88 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.38)] transition-all duration-200 {commentsPanelOpen ? 'md:translate-y-0 md:opacity-100' : 'md:translate-y-6 md:opacity-0 md:pointer-events-none'} {commentsPanelMode === 'full' ? 'md:max-h-[85vh]' : ''}">
            ...
            <div class="hidden fixed md:static left-2 right-2 md:left-auto md:right-auto bottom-[max(0px,env(safe-area-inset-bottom))] z-40 p-2 rounded-t-xl md:rounded-none bg-[#0f1728]/92 backdrop-blur-md shadow-[0_-10px_24px_rgba(0,0,0,0.28)] md:shadow-none">
                <CommentInput bind:this={commentInput} onbuttonclicked={onCommentInputButton} />
            </div>

            {#if !commentsPanelOpen}
                <button
                    class="hidden absolute z-20 right-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] items-center gap-2 rounded-full bg-[#0c1018]/92 border border-slate-700/80 px-3 py-2 text-sm text-slate-200 shadow-xl"
