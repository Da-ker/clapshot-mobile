...
let desktopLongPressSeeking = $state(false);
let desktopMuted = $state(false);
let desktopVolume = $state(1);
const desktopControlInnerBtnClass = 'h-10 w-10 rounded-full inline-flex items-center justify-center text-slate-100 transition-colors hover:bg-slate-600/55';
let useDesktopReviewLayout = $state(false);
...
function refreshTopVideoMeta() {
    ...
    desktopLongPressSeeking = longPressSeeking;
...
function onDesktopStepBackwardRelease(e: MouseEvent | PointerEvent) {
    ...
    videoPlayer?.endStepButtonLongPress?.(-1);
    refreshTopVideoMeta();
}
...
function detectDesktopReviewLayout() {
...
$effect(() => {
    const applyReviewLayout = () => {
        useDesktopReviewLayout = detectDesktopReviewLayout();
    };
    applyReviewLayout();
    window.addEventListener('resize', applyReviewLayout);
    window.visualViewport?.addEventListener('resize', applyReviewLayout);
    return () => {
        window.removeEventListener('resize', applyReviewLayout);
        window.visualViewport?.removeEventListener('resize', applyReviewLayout);
    };
});
...
.then(json => {
    ...
    connectWebsocket(json.ws_url, (json.skip_auth_check ?? false));
...
function connectWebsocket(wsUrl: string, skipAuthCheck: boolean = false) {
    const http_health_url = wsUrl.replace(/^wss:/, "https:").replace(/^ws:/, "http:").replace(/\/api\/.*$/, "/api/health");

    if (skipAuthCheck) {
        console.warn("SKIP_AUTH_CHECK enabled: skipping /api/health auth check and connecting directly to WS API");
...
<main>
    ...
    <div class="safe-area-pad box-border flex flex-col bg-[#101016] w-full h-[100dvh] overflow-hidden {debugLayout?'border-2 border-yellow-300':''}">
        ...
        <div bind:this={videoReviewRootEl} class={`relative h-full min-h-0 overflow-hidden ${useDesktopReviewLayout ? 'flex flex-col' : ''}`}>
            <div class={`px-2 pt-2 pb-0 ${useDesktopReviewLayout ? 'hidden' : ''}`}>
                <div class="rounded-xl bg-slate-900/55 px-2.5 md:px-3 py-[2px] backdrop-blur-sm">
                    <div class="top-info-row flex flex-nowrap items-center gap-1.5 text-sm md:text-base min-w-0">
                        ...
                        <span class="rounded-lg bg-slate-800/55 px-2 py-[2px] text-slate-200 ml-auto shrink-0 text-sm md:text-base font-semibold">⏱ {formatDurationShort($curVideo?.duration?.duration)}</span>
                        <span class="rounded-lg bg-slate-800/55 px-2 py-[2px] text-slate-200 shrink-0 text-sm md:text-base font-semibold">{$curVideo?.duration?.fps ?? '-'} fps</span>
                    </div>
                </div>
            </div>
            <div class={`w-full px-2 pb-0 ${useDesktopReviewLayout ? 'px-6 flex-1 min-h-0' : ''}`} style="margin-top: {LAYOUT_SECTION_GAP_PX}px;" data-video-stack>
                <div class={`w-full max-w-[1400px] mx-auto ${useDesktopReviewLayout ? 'w-fit max-w-none grid grid-cols-[minmax(0,min(calc((100dvh-150px)*16/9),calc(100vw-420px)))_clamp(260px,24vw,380px)] gap-[20px] items-stretch justify-center h-full min-h-0 max-h-full' : ''}`}>
                    <div class={`min-w-0 ${useDesktopReviewLayout ? 'min-h-0 h-full flex flex-col' : ''}`}>
                        <div bind:this={videoPanelEl} class={`w-full rounded-2xl bg-[#0b1220]/88 p-0 shadow-[0_14px_36px_rgba(0,0,0,0.35)] overflow-hidden ${useDesktopReviewLayout ? 'min-h-0 flex-1 max-h-[calc(100dvh-150px)]' : ''}`}>
                            <VideoPlayer
                        ...
                        <div class={`${useDesktopReviewLayout ? 'block' : 'hidden'} mt-2`}>
                                ...
                                <div class="top-info-row flex flex-nowrap items-center gap-1.5 text-sm min-w-0">
                                    <span class="top-left-controls inline-flex flex-1 items-center rounded-lg bg-slate-800/45 px-2 py-[2px] text-slate-100 font-mono text-sm min-w-0">
                                        ...
                                        <span class="inline-flex flex-1 items-center justify-center ml-4 gap-3">
                                            <button class="h-12 w-12 min-h-12 min-w-12 rounded-full inline-flex items-center justify-center text-slate-100 border border-slate-500/55 transition-colors {desktopLongPressSeeking ? 'bg-slate-700/45 hover:bg-slate-600/55' : (desktopPaused ? 'bg-slate-700/45 hover:bg-slate-600/55' : 'bg-slate-500/55 hover:bg-slate-400/65')}" onclick={onDesktopTogglePlayPause} aria-label="Play/Pause">
                                            ...
                                            <span class="relative h-12 w-[188px] inline-flex justify-start">
                                                <span class="group relative h-12 w-12 hover:w-[188px] rounded-full bg-slate-800/55 border border-slate-500/45 overflow-hidden transition-[width] duration-220 ease-out">
                                                    <div class="absolute inset-y-0 left-0 w-12 flex items-center justify-center z-10">
                                                        <button class={desktopControlInnerBtnClass} onclick={onDesktopToggleMute} aria-label="Mute/Unmute">
                                                            <i class="fa-solid {desktopMuted ? 'fa-volume-xmark' : 'fa-volume-high'} text-[17px]"></i>
                                                        </button>
                                                    </div>
                                                    <div class="absolute inset-y-0 left-12 flex items-center origin-left transition-all duration-220 ease-out opacity-0 scale-x-0 group-hover:opacity-100 group-hover:scale-x-100 w-[126px] pl-2 pr-2">
                        ...
                        <div bind:this={commentInputDockEl} class={`fixed inset-x-0 bottom-[max(0px,env(safe-area-inset-bottom))] z-30 pt-0 pb-2 rounded-t-xl bg-[#0f1728]/92 backdrop-blur-md shadow-[0_-10px_24px_rgba(0,0,0,0.28)] ${useDesktopReviewLayout ? 'static mt-2 w-full max-w-none mx-auto left-auto right-auto bottom-0 shrink-0 rounded-xl shadow-[0_-10px_24px_rgba(0,0,0,0.28)]' : ''}`}>
                    ...
                    <div
                        ...
                        class={`absolute z-20 inset-x-0 mx-auto w-full max-w-[1400px] px-2 top-[var(--comments-top-px)] bottom-[calc(var(--mobile-comment-input-h)+env(safe-area-inset-bottom))] flex flex-col min-h-0 overflow-hidden rounded-t-2xl bg-[#0f1728]/88 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.38)] transition-all duration-200 ${useDesktopReviewLayout ? 'static inset-auto mx-0 w-full max-w-none h-[var(--desktop-comments-h)] min-h-0 rounded-xl px-0 top-auto bottom-auto translate-y-0 opacity-100 pointer-events-auto' : ''}`}
                    ...
                    {:else if pit.folderListing}
                        <div class="my-4 xl:my-6">
                            ...
                            {#if pit.folderListing.allowUpload}
                                <div class="h-24 border-2 xl:border-4 border-dashed border-gray-700 rounded-md">
                                    <FileUpload
                                        postUrl={uploadUrl}
