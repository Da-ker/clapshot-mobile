                                ...
                                <div class="top-info-row flex flex-nowrap items-center gap-1.5 text-sm min-w-0">
                                    <span class="top-left-controls inline-flex flex-1 items-center rounded-lg bg-slate-800/45 px-2 py-[2px] text-slate-100 font-mono text-sm min-w-0">
                                        ...
                                        <span class="inline-flex flex-1 items-center justify-center ml-4 gap-3">
                                            <button class="h-12 w-12 min-h-12 min-w-12 rounded-full inline-flex items-center justify-center text-slate-100 border border-slate-500/55 transition-colors {(desktopPaused || desktopLongPressSeeking) ? 'bg-slate-700/45 hover:bg-slate-600/55' : 'bg-slate-500/55 hover:bg-slate-400/65'}" onclick={onDesktopTogglePlayPause} aria-label="Play/Pause">
                                                <i class="fa-solid {(desktopPaused || desktopLongPressSeeking) ? 'fa-play' : 'fa-pause'} text-[18px]"></i>
                                            </button>
                                    ...
                                    <span class="rounded-lg bg-slate-800/40 px-2 py-[2px] text-slate-200 ml-auto shrink-0 text-sm font-semibold">LP={desktopLongPressSeeking ? 'ON' : 'OFF'}</span>
                                    <span class="rounded-lg bg-slate-800/40 px-2 py-[2px] text-slate-200 shrink-0 text-sm font-semibold">⏱ {formatDurationShort($curVideo?.duration?.duration)}</span>
